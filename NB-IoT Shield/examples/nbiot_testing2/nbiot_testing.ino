...
#define DHT11_PIN A0    //define hpt11 data pin

float temperature,tem,humidity,hum; //define variable for dpt11

void mcu_init();        //CPU init 
...
*****************************************************************************************/
void loop() {
  read_dht();                        //detect the humidity and temperature
  if((tem >=30)||(hum >= 80))        //when temperaturn beyond 30℃ or humidity beyond 80%, Arduino send command to plotform      
  {
    ...
    mcu_at_nmgs(3, "020103");                                                 //send message to CoAP server, for testing    delay(timer);                                                             //delay(1500)
    read_downstream();
  }                   
}
/**************************************************************************************
 * funciton name  ：read_dht()
 * parameter      ：NONE
 * return value   ：NONE 
 * effect         ：read the dpt11 humidity and temperature
 * data           ：2018/4/16
*****************************************************************************************/
void read_dht()
{
